...
public class JDBCThirdProgram {
            ...
            while (true) {
                ...
                switch (choice) {
                    ...
                    default:
                        ...
                        // Exit the loop on invalid choice
                        return;
                }
            }
    ...
    public static void fetchAllData(Connection con) {
        try {
    ...
    public static void fetchData(Connection con) {
        try {
            Scanner scanner = new Scanner(System.in);
            System.out.println("Enter course ID to fetch:");
            int c_id = scanner.nextInt();

            String queryString = "SELECT * FROM course WHERE c_id = ?";
            PreparedStatement pstmt = con.prepareStatement(queryString);
            pstmt.setInt(1, c_id);
            ResultSet rs = pstmt.executeQuery();
            if (rs.next()) {
                ...
                String trainer = rs.getString("c_trainer");
                int cost = rs.getInt("c_cost");
                System.out.println("ID: " + id + ", Name: " + name + ", Trainer: " + trainer + ", Cost: " + cost);
            } else {
                System.out.println("No data found for ID: " + c_id);
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
